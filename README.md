...
「今日の記録」の各食事の「📷 写真」ボタンから、料理写真を撮影してメニュー名と
栄養値をAIで推定し、確認・修正して記録できる。認識した料理は食品一覧にも自動登録され、
次回から手入力検索でも使える。

- 1回の撮影で扱うのは写真の中心にある皿1つだけで、材料に分解せず1つの料理メニューとして
  1品だけ提示する(端に映り込んだ別の皿や付け合わせは無視)
- 量は写真から推定せず、常に100gの仮置きで表示する。確認画面で実測した皿の重量に修正すると
  栄養値が比例換算される(AIは100gあたりの栄養値を返す)
...
消費カロリーは、基礎代謝 2000 kcal/日(固定)に、運動管理アプリに記録したその日のジョギング時間を
1時間400kcal換算した運動カロリーを足した値です。日ごとに独立して計算され、分析画面の日別表に
運動カロリーの列があります。
...
同一オリジン(https://taka070600538-tech.github.io/)で公開されている「運動管理アプリ」と
IndexedDB(`exercise-app-db`)を共有しており、ダッシュボードや分析画面を表示するたびに
日付単位で `jogging.durationMin` を読み取ってその日の運動カロリーに換算します。
カロリー計算アプリ側には運動カロリーのコピーを保存しません。

- 同じ端末・同じブラウザで両方のアプリを使う必要があります。
- 運動管理アプリのデータが見つからない場合は、運動を0kcalとして計算し、画面にその旨を注記します(エラーで止まりません)。
- 今日の記録(ダッシュボード)にも、その日の消費カロリーと収支が表示されます。
